...
public class MemoryUtil {
    ...
    public static void printMemoryInfo(Context context) {
        ...
        NLog.i(TAG, "memory: native heap alloc: " + Debug.getNativeHeapAllocatedSize());
        ...
        for (RunningAppProcessInfo i : runningAppProcesses) {
            if (i.processName.equals(context.getPackageName())) {
                Debug.MemoryInfo[] mem = activityManager.getProcessMemoryInfo(new int[]{i.pid});
                NLog.i(TAG, i.processName + " pss:" + (mem[0].getTotalPss() / 1024) + "MB");
            }
        }
        try {
