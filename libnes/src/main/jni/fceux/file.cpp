...
static std::string BatterySaveDir;
...
void NOSTALGIA_SetBatterySaveDir(std::string const & dir) {
	BatterySaveDir = dir;
}
...
std::string FCEU_MakeFName(int type, int id1, const char *cd1)
{
	...
	switch(type)
	{
		...
		case FCEUMKF_SAV:
			...
			sprintf(ret,"%s/%s.%s",/*FileBaseDirectory*/BatterySaveDir.c_str(), FileBase,cd1);
			/*if(odirs[FCEUIOD_NV])
